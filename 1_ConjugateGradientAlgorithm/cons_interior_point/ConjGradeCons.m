...
% i.e. PR-CG algorithm
% Adopt interior penalty function method to deal with constraints
...
default.penalFac        = 1;            % Intial penalty factor
default.iterFac         = 2;            % Iteration factor
...
default.stepDist        = 0.1;          % 1D search distance
...
optStep.epsilon  = options.stepErr;
optStep.distance = options.stepDist;
...
info.penVal         = zeros(1, maxIterOut);         % Penalty function value
...
p1 = 1;
x1 = x0 + 1;
while ((r * p1 > epsilonOut) || (max(abs(x1 - x0)) > epsilonOut)) ...
        && (iterOut <= maxIterOut)
    ...
    while (max(abs(gVal)) > epsilonIn) && (iterIn <= maxIterIn)
        ...
        [xVal, funVal] = StepOptim(xVal, dVal, xLb, xUb, r, Ct, Fs, optStep);
        
        % Calculate partial differential
        xInc = repmat(xVal, 1, D) + xDel;
        funValInc = EnhObjFun(xInc, xLb, xUb, r, Ct, Fs);
        gVal0 = gVal;
    ...
    if strcmp('iter', options.display)
        if mod(iterOut - 1, options.printMod) == 0
            fprintf(['iter: %3d,  freq: %9.3e,  pha: %9.3e  objFun: %9.3e  ' ...
                'penFun: %9.3e  freqGrad: %9.3e  phaGrad: %9.3e\n'],...
                iterOut, info.freqVal(iterOut), info.phaVal(iterOut), info.funVal(iterOut), ...
                info.penVal(iterOut), info.gradFreq(iterOut), info.gradPha(iterOut));
        end
    end
...
[~, R] = size(X);
XLb = repmat(xLb, 1, R);                    % D*R
XUb = repmat(xUb, 1, R);                    % D*R

% Find interior and exterior points
% gPlusTemp = [(X - XUb); (XLb -X)];          % D*R
% intIdx = gPlusTemp < 0;
gPlus = log(-[(X - XUb); (XLb -X)]);        % D*R

% Calculate the value of penalty function
penFun = -sum(gPlus);                       % 1*R

% Calculate output value of enhanced obejct function
y = ObjFun(X, Ct, Fs) + r * penFun;
